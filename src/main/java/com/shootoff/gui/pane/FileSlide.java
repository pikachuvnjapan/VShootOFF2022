...
public class FileSlide extends Slide {
	...
	public FileSlide(Pane parentControls, Pane parentBody, CalibrationConfigurator calibrationConfigurator,
		...
		addSlideControlButton("Cài Đặt", (event) -> {
			...
			preferencesSlide.showBody();
                      ...
                      //
						/*
						 * addSlideControlButton("Lưu Hình Ảnh", (event) -> { final Node container =
						 * cameraViews.getSelectedCameraContainer(); final RenderedImage renderedImage =
						 * SwingFXUtils .fromFXImage(container.snapshot(new SnapshotParameters(), null),
						 * null);
						 * 
						 * final FileChooser fileChooser = new FileChooser();
						 * fileChooser.setTitle("Lưu Hình Ảnh");
						 * fileChooser.getExtensionFilters().addAll( new
						 * FileChooser.ExtensionFilter("Graphics Interchange Format (*.gif)", "*.gif"),
						 * new FileChooser.ExtensionFilter("Portable Network Graphic (*.png)",
						 * "*.png"));
						 * 
						 * final File feedFile =
						 * fileChooser.showSaveDialog(parentControls.getScene().getWindow());
						 * 
						 * if (feedFile != null) { final String extension =
						 * fileChooser.getSelectedExtensionFilter().getExtensions().get(0).substring(2);
						 * File imageFile;
						 * 
						 * if (feedFile.getPath().endsWith(extension)) { imageFile = feedFile; } else {
						 * imageFile = new File(feedFile.getPath() + "." + extension); }
						 * 
						 * try { ImageIO.write(renderedImage, extension, imageFile); } catch (final
						 * IOException e) { logger.error("Error saving feed image", e); } } });
						 */
                        //Exit
		addSlideControlButton("Thoát Phần Mềm", (evnet) -> {
			mainWindow.close();
		});
	}
}

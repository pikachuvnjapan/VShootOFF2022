...
public class ExerciseSlide extends Slide implements PluginListener, ItemSelectionListener<TrainingExercise> {
	...
	public ExerciseSlide(Pane parentControls, Pane parentBody, ExerciseListener exerciseListener) {
		...
		addSlideControlButton("Chọn Bài Tập", (event) -> {
		...
		addSlideControlButton("Ghi Lại Bài Tập", (event) -> {
			if (config.getSessionRecorder().isPresent()) {
				stopRecordingSession();

				((Button) event.getSource()).setText("Ghi Lại Bài Tập");
			} else {
				startRecordingSession();

				((Button) event.getSource()).setText("Dừng Ghi Bài Tập");
			}

			hide();
		});

		addSlideControlButton("Xem Bài Tập", (event) -> {
		...
		final TitledPane universalPane = new TitledPane("Bài Tập Phổ Thông", exerciseItemPane);
		projectorPane = new TitledPane("Bài Tập Theo Đề Án", projectorExerciseItemPane);
